...
#    Floor is nextgenCrossDesign >= 0.25.0 (inst/BACKEND_VERSION; a config.yml
#    required_backend_version: entry, if you set one, overrides it).
remotes::install_github("pulsesmartlab-innovations/nextgenCrossDesignR@v0.25.0")

#    …or from a local source tarball:
R CMD INSTALL nextgenCrossDesign_0.25.0.tar.gz

# 2. this front-end — from CRAN once published:
install.packages("nextgenCrossWorkbench", dependencies = TRUE)

#    …or from a local source tarball:
install.packages("nextgenCrossWorkbench_0.28.0.tar.gz",
...
Rscript tools/update-backend.R 0.25.0   # or a specific version
```
...
```r
NGCD_BACKEND_TARBALL=/path/nextgenCrossDesign_0.25.0.tar.gz Rscript tools/update-backend.R
```
...
> front-end for nextgenCrossDesign* (R package, v0.28.0). North Dakota State
