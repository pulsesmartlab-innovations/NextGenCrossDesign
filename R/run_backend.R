...
ngcd_error_hint <- function(msg) {
  ...
  if (grepl("residual-heterozygosity|assume_inbred|inbred", msg))
    return("Parents look non-inbred for a DH/RIL model. Use homozygous 0/2 dosages, or uncheck 'Assume inbred parents' on Scoring.")
  ...
  if (grepl("lpsolve|mip", msg))
    return("A MIP optimizer needs the 'lpSolve' package, or pick optimizer = evolution/greedy on Allocation.")
  ...
  if (grepl("could not find function|there is no package|namespace", msg))
    return("The backend package may be missing or in a different library. Check Setup and package_library in config.yml.")
  NULL
}
