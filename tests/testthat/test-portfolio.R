...
test_that("ngcd_portfolio_plotly shows per-cross confidence in hover, not one recycled value", {
  res <- list(settings = list(selection_prop = 0.1),
    selected_crosses = data.frame(
      cross = c("A/B","C/D","E/F"), cross_level = c(3,2,1), cross_upside = c(2,1,2),
      cross_confidence = c(.9,.5,.7),
  ...
  marker_traces <- Filter(function(tr) identical(tr$mode, "markers"), pb$x$data)
  txt <- unlist(lapply(marker_traces, function(tr) tr$text))
  expect_true(any(grepl("0.90", txt, fixed = TRUE)))
  expect_true(any(grepl("0.50", txt, fixed = TRUE)))
  expect_true(any(grepl("0.70", txt, fixed = TRUE)))
})

test_that("ngcd_portfolio_plotly does not error when risk_bin has NA", {
  res <- list(settings = list(selection_prop = 0.1),
    selected_crosses = data.frame(
      cross = c("A/B","C/D","E/F"), cross_level = c(3,2,1), cross_upside = c(2,1,2),
      cross_confidence = c(.9,.5,.7),
      risk_bin = factor(c("low", NA, NA), levels = c("low","med","high"), ordered = TRUE),
      portfolio_profile = factor(c("breakthrough","long_shot","workhorse")),
      priority_tier = factor(c("highly_priority","priority","medium_priority")),
      stringsAsFactors = FALSE))
  p <- nextgenCrossWorkbench:::ngcd_portfolio_plotly(res)
...
test_that("ngcd_portfolio_plotly does not error when an axis column is entirely NA", {
  res <- list(settings = list(selection_prop = 0.1),
    selected_crosses = data.frame(
      cross = c("A/B","C/D","E/F"), cross_level = c(3,2,1), cross_upside = rep(NA_real_, 3),
      cross_confidence = c(.9,.5,.7),
      risk_bin = factor(c("low","high","med"), levels = c("low","med","high"), ordered = TRUE),
      portfolio_profile = factor(c("breakthrough","long_shot","workhorse")),
      priority_tier = factor(c("highly_priority","priority","medium_priority")),
      stringsAsFactors = FALSE))
  p <- suppressWarnings(nextgenCrossWorkbench:::ngcd_portfolio_plotly(res))
  expect_true(inherits(p, "plotly") || inherits(p, "htmlwidget"))
})
