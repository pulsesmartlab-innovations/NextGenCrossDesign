...
```
ghcr.io/pulsesmartlab-innovations/ngcd-workbench:0.20.0   # (also :latest)
```
...
#    -> produces nextgenCrossDesign_0.17.2.tar.gz
R CMD build /path/to/nextgenCrossDesignR

# 2. Build the front-end tarball (from a checkout of THIS repo)
#    -> produces nextgenCrossWorkbench_0.20.0.tar.gz
R CMD build .

# 3. Put both tarballs at the repo root (the build context) and build the image
docker build -t ngcd-workbench:0.20.0 \
  --build-arg BACKEND_TARBALL=nextgenCrossDesign_0.17.2.tar.gz \
  --build-arg FRONTEND_TARBALL=nextgenCrossWorkbench_0.20.0.tar.gz \
  -f deploy/Dockerfile .
```
...
docker buildx build --builder ngcd-builder \
  ...
  -t ghcr.io/pulsesmartlab-innovations/ngcd-workbench:0.20.0 \
